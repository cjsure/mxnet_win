...
//    set_updater(nullptr);
  ...
  void Push(const std::vector<int>& keys,
    ...
    for (size_t i = 0; i < uniq_keys.size(); ++i) {
      ...
      if (updater_ != nullptr) {
        ...
        NDArray* its = &(it->second);
        updater_(key, merged, its);
      }
    }
  }
 ...
 protected:
  ...
  // updater
  //Updater updater_;
  std::function<void(int, const NDArray&, NDArray*)> updater_;
  std::vector<std::function<void(int, const NDArray&, NDArray*)>> updater_vector_;
