...
void ThreadedEngine::DeleteOperator(OprHandle op) {
  ...
  std::function<void(RunContext)> fc = [threaded_opr](RunContext) {
    ThreadedOpr::Delete(threaded_opr);
  };
  this->PushSync(fc, Context::CPU(), {}, deps, FnProperty::kAsync);
}
...
void ThreadedEngine::DeleteVariable(SyncFn delete_fn,
  ...
  std::function<void(RunContext)> fc = [delete_fn, threaded_var](RunContext ctx) {
    // Mark variable as orphan,
    // so during `ThreadedEngine::OnComplete` it could be recycled.
    threaded_var->SetToDelete();
    delete_fn(ctx);
  };
  this->PushSync(fc, exec_ctx, {}, {var}, FnProperty::kAsync);
}
...
void ThreadedEngine::WaitForVar(VarHandle var) {
  ...
  std::function<void(RunContext)> fc = [this, &done](RunContext) {
    if (engine_info_) {
  ...
  this->PushSync(fc, Context::CPU(), {var}, {}, FnProperty::kNormal);
  {
    std::unique_lock<std::mutex> lock{finished_m_};
