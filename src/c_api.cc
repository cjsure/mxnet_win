...
int MXKVStorePush(KVStoreHandle handle,
  ...
  auto kv_handle = (static_cast<KVStore*>(handle));
  kv_handle->Push(v_keys, v_vals, priority);
  API_END();
}
...
int MXKVStoreSetUpdater(KVStoreHandle handle, MXKVStoreUpdater updater) {
  ...
  */
  auto current_updater = std::move(updater);
  auto updt = [current_updater](int key, const NDArray& recv, NDArray* local) {
    NDArray* recv_copy = new NDArray();
    *recv_copy = recv;
    NDArray* local_copy = new NDArray();
    *local_copy = *local;
    current_updater(key, recv_copy, local_copy);
    };
