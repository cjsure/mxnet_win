...
void GraphExecutor::InitGraph(const Symbol &symbol, Context ctx, bool need_backward) {
  ...
  for (uint32_t nid : topo) {
    if (nid < num_forward_nodes_) {
      ...
      backward.push_back(nid);// NOTE(Jiahui Yu): backward logic
    }
  }
  // NOTE(Jiahui Yu): Connect forward and backward
...
void GraphExecutor::InitDataEntryInfo(const std::vector<NDArray> &in_args, // NOTE(Jiahui Yu): Data entry reference count and so on.
