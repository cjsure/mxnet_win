...
void SetValueOp(const real_t &rhs, NDArray *out) {
  ...
  switch (ret.ctx().dev_mask()) {
    case cpu::kDevMask: {
                          std::function<void(RunContext)> fc = [rhs, ret](RunContext ctx) {
                            ret.CheckAndAlloc();
                            TBlob tmp = ret.data();
                            ndarray::Eval<cpu>(rhs, &tmp, ctx);
                          };
      Engine::Get()->PushSync(fc, ret.ctx(), {}, {ret.var()});
      break;
    }
#if MXNET_USE_CUDA
    case gpu::kDevMask: {
...
void SampleOP(const real_t &a,
  ...
  switch (out->ctx().dev_mask()) {
    case cpu::kDevMask: {
                          std::function<void(RunContext)> fc = [a, b, resource, ret](RunContext ctx) {
                            ret.CheckAndAlloc();
                            TBlob tmp = ret.data();
                            ndarray::EvalRandom<cpu, Distribution>(a, b, resource, &tmp, ctx);
                          };
      Engine::Get()->PushSync(fc, out->ctx(), {}, {ret.var(), resource.var});
      break;
    }
#if MXNET_USE_CUDA
    case gpu::kDevMask: {
...
void NDArray::SyncCopyToCPU(real_t *data, size_t size) const {
  ...
  //RunContext run_ctx;
  //NOTE(Jiahui Yu): In debug mode, the run_ctx is used without initialized
